import { useState } from "react";
import { useNavigate, Link } from "react-router-dom";
import { authClient } from "../lib/auth-client";
import Button from "../components/Button";
import logo from "../Images/Logo.png";

const Connexion = () => {
  // States pour stocker les valeurs du formulaire
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  // State pour afficher un message d'erreur
  const [error, setError] = useState("");
  // State pour afficher un état de chargement pendant la connexion
  const [loading, setLoading] = useState(false);

  // Hook React Router pour rediriger l'utilisateur
  const navigate = useNavigate();

  // Fonction appelée lors de la soumission du formulaire
  const handleSubmit = async (e: React.FormEvent) => {
    // Empêche le rechargement de la page
    e.preventDefault();
    // Réinitialisation des messages
    setError("");
    setLoading(true);
    // Appel au système d'authentification avec email + mot de passe
    const { error } = await authClient.signIn.email({ email, password });
    // Si une erreur survient
    if (error) {
      setError(error.message || "Erreur lors de la connexion");
      setLoading(false);
      return;
    }
    // Si la connexion fonctionne
    setLoading(false);
    // Redirection vers la page d'accueil
    navigate("/");
  };

  // Fonction pour se connecter avec Google
  const handleGoogle = async () => {
    // Réinitialise les erreurs
    setError("");
    // Lance la connexion OAuth Google
    await authClient.signIn.social({
      provider: "google",
      callbackURL: "http://localhost:5173/",
    });
  };

  return (
    // Conteneur principal centré sur la page
    <div className="min-h-screen flex items-center justify-center bg-[#9C9475]">
      <form
        onSubmit={handleSubmit}
        className="bg-white rounded-2xl p-14 w-full max-w-lg flex flex-col gap-6 shadow-lg"
      >
        <div className="flex justify-center mb-2">
          <img src={logo} alt="Rézine'Za" className="h-36" />
        </div>
        <p className="font-['Lato'] text-base text-center text-gray-500">
          Se connecter
        </p>
        {error && (
          <p className="font-['Lato'] text-sm text-red-500 text-center">
            {error}
          </p>
        )}
        <input
          type="email"
          placeholder="Email"
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          required
          className="font-['Lato'] border border-gray-300 rounded-lg px-4 py-3 outline-none focus:border-[#405882] text-sm"
        />
        <input
          type="password"
          placeholder="Mot de passe"
          value={password}
          onChange={(e) => setPassword(e.target.value)}
          required
          minLength={8}
          className="font-['Lato'] border border-gray-300 rounded-lg px-4 py-3 outline-none focus:border-[#405882] text-sm"
        />
        <Button
          text={loading ? "..." : "Se connecter"}
          type="submit"
          width="w-full"
          radius="rounded-lg"
        />
        <div className="flex items-center gap-3">
          <div className="h-px flex-1 bg-gray-300" />
          <span className="font-['Lato'] text-xs text-gray-400">ou</span>
          <div className="h-px flex-1 bg-gray-300" />
        </div>
        <button
          type="button"
          onClick={handleGoogle}
          className="flex items-center justify-center gap-2 border border-gray-300 rounded-lg px-4 py-3 cursor-pointer hover:bg-gray-50 font-['Lato'] text-sm"
        >
          <svg width="18" height="18" viewBox="0 0 24 24">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>

          Continuer avec Google
        </button>
        <Link
          to="/inscription"
          className="font-['Lato'] text-sm text-gray-500 hover:text-[#405882] text-center"
        >
          Pas de compte ? S'inscrire
        </Link>
        <Link
          to="/"
          className="font-['Lato'] text-sm text-[#405882] hover:underline text-center"
        >
          ← Retour à l'accueil
        </Link>

      </form>
    </div>
  );
};

export default Connexion;